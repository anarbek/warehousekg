...
## Invoices Module — `/api/v1/invoices`

> **Auth**: All endpoints require `[Authorize(Policy = "invoices:{action}")]`.
> Invoices are auto-created when a delivery stop with shipments is completed (`CompleteDeliveryStopCommandHandler`).

### Frontend routes (Angular)

| Angular route | Component |
|---|---|
| `/invoices/list` | `InvoiceList` — data grid with filters (status, date range), grand total column |
| `/invoices/new` | `InvoiceForm` (create mode) — customer/warehouse/item dropdowns, live totals |
| `/invoices/:id` | `InvoiceDetail` — workflow buttons, grid with column sums, PDF print |
| `/invoices/:id/edit` | `InvoiceForm` (edit mode) |

The `InvoiceService` wraps `/api/v1/invoices`.

### Endpoints

| Method | Route | Description | Auth |
|--------|-------|-------------|------|
| `GET` | `/api/v1/invoices` | List invoices (filters: status, customerId, warehouseId, salesOrderId, dateFrom, dateTo) | `invoices:read` |
| `GET` | `/api/v1/invoices/{id}` | Get invoice detail with lines and resolved names | `invoices:read` |
| `GET` | `/api/v1/invoices/{id}/pdf` | Get printable HTML invoice (column sums in `<tfoot>`) | `invoices:read` |
| `POST` | `/api/v1/invoices` | Create invoice (Draft) | `invoices:write` |
| `PUT` | `/api/v1/invoices/{id}` | Update invoice lines (Draft only) | `invoices:write` |
| `POST` | `/api/v1/invoices/{id}/issue` | Issue invoice (Draft → Issued) | `invoices:write` |
| `POST` | `/api/v1/invoices/{id}/print` | Mark as Printed (Issued/Printed → Printed) | `invoices:write` |
| `POST` | `/api/v1/invoices/{id}/sign` | Sign invoice (Issued/Printed → Signed, body: `{ signedByName }`) | `invoices:write` |
| `POST` | `/api/v1/invoices/{id}/cancel` | Cancel invoice (Issued/Printed → Cancelled, NOT Signed) | `invoices:write` |
| `DELETE` | `/api/v1/invoices/{id}` | Delete invoice and all lines | `invoices:delete` |

### Invoice status lifecycle

```
Draft → Issue → Issued → Print → Printed → Sign → Signed
                      ↘ Cancel ↘ Cancelled
                      Print ↗
```

- **Cannot cancel** a `Signed` invoice (returns `409 Conflict`)
- **Cannot issue** an already-issued invoice (returns `409 Conflict`)
- **Cannot sign** a `Draft` invoice (must be issued/printed first)
...
```json
{
  ...
  "dueDateUtc": "2026-06-24T00:00:00Z",
  "notes": "string|null",
  "lines": [
    {
...
> **taxRate**: User enters `2` meaning 2%. Backend divides by 100: `TaxAmount = lineTotal * taxRate / 100m`.

### Numbering

Sequential per-tenant: `INV-{YEAR}-{NNNN}` (e.g. `INV-2026-0001`). Auto-generated by `CreateInvoiceCommand`.

### InvoiceDto response shape
...
```json
{
  ...
  "status": "Draft",
  "customerId": "guid",
  "customerName": "SAMARKAND",
  "warehouseId": "guid",
  "warehouseName": "depo 2",
  "issuedAtUtc": "2026-06-22T20:18:00Z",
  "printedAtUtc": null,
  "signedAtUtc": null,
  "dueDateUtc": null,
  "totalAmount": 28.00,
...
> **totalAmount** = sum of line totals (subtotal WITHOUT tax). Grand total = `totalAmount + taxAmount`.

### PDF endpoint

`GET /api/v1/invoices/{id}/pdf` returns `text/html; charset=utf-8`.

Table columns: Товар | Кол-во | Цена | Сумма | Налог % | Сумма налога | Итого  
Column sums in `<tfoot>` under Кол-во, Сумма, Сумма налога, Итого.

Frontend downloads via `HttpClient` (Blob) → `URL.createObjectURL()` → `window.open()`. This ensures the JWT auth header is sent; using `window.open(url)` directly would return `401`.
