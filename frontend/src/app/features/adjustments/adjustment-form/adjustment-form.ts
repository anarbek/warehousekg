...
export class AdjustmentForm implements OnInit {
  private readonly fb=inject(FormBuilder);private readonly svc=inject(AdjustmentsService);private readonly inv=inject(InventoryService);private readonly personnel=inject(PersonnelService);private readonly router=inject(Router);private readonly route=inject(ActivatedRoute);
  ...
  protected formData:any={number:'',warehouseId:'',reason:0,notes:'',employeeId:null};
  ...
  ngOnInit(){forkJoin({w:this.inv.getWarehouses(),i:this.inv.getInventoryItems(),e:this.personnel.getEmployees()}).subscribe({next:d=>{
    this.items.set(d.i);
    this.headerItems.set([
      ...
      {dataField:'employeeId',editorType:'dxSelectBox',label:{text:'Сотрудник (опц.)'},editorOptions:{dataSource:d.e,displayExpr:'lastName',valueExpr:'id',placeholder:'Не указан',stylingMode:'outlined'}},
  ...
  submit(){this.saving.set(true);this.err.set(null);
    this.svc.createAdjustment({number:this.formData.number,warehouseId:this.formData.warehouseId,reason:Number(this.formData.reason),notes:this.formData.notes||null,employeeId:this.formData.employeeId||null,lines:this.form.getRawValue().lines.map((l:any)=>({inventoryItemId:l.inventoryItemId!,quantityChange:Number(l.quantityChange),notes:l.notes||null}))}).subscribe({next:()=>{this.saving.set(false);void this.router.navigate(['..'],{relativeTo:this.route})},error:()=>{this.err.set($localize`:@@common.saveError:Не удалось сохранить данные`);this.saving.set(false)}})}
