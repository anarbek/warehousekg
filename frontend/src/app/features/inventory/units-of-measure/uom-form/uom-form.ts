...
@Component({selector:'app-uom-form',imports:[DxFormModule,DxButtonModule,DxProgressBarModule],templateUrl:'./uom-form.html',styleUrl:'./uom-form.scss'})
export class UomForm implements OnInit {
  ...
  protected readonly title=this.isEdit?'Edit unit':'New unit';
  ...
  ngOnInit(){if(!this.isEdit)return;this.loading.set(true);this.svc.getUnitOfMeasureById(this.editId!).subscribe({next:c=>{this.formData=c;this.loading.set(false)},error:()=>{this.err.set('Load error');this.loading.set(false)}})}
  submit(){this.saving.set(true);this.err.set(null);
    ...
    (this.isEdit?(this.svc.updateUnitOfMeasure(this.editId!,this.formData)as any):(this.svc.createUnitOfMeasure(this.formData)as any)).subscribe({next:d,error:f})}
