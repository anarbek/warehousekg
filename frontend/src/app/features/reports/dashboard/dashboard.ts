...
export class Dashboard implements OnInit {
  ...
  ngOnInit():void{this.loading.set(true);forkJoin({inv:this.svc.getInventorySummary(),sales:this.svc.getSalesSummary(),purch:this.svc.getPurchaseSummary(),mov:this.svc.getStockMovements()}).subscribe({next:d=>{this.inv.set(d.inv);this.sales.set(d.sales);this.purch.set(d.purch);this.mov.set(d.mov);this.loading.set(false)}})}
}
