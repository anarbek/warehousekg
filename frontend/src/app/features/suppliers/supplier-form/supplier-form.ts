...
export class SupplierForm implements OnInit {
  private readonly svc=inject(SupplierPoService);private readonly route=inject(ActivatedRoute);private readonly router=inject(Router);private readonly toast=inject(ErrorToastService);
  ...
  ngOnInit(){if(!this.isEdit)return;this.loading.set(true);this.svc.getSupplierById(this.editId!).subscribe({next:s=>{this.formData=s;this.loading.set(false)},error:(e)=>{this.toast.showLoad(e);this.loading.set(false)}})}
