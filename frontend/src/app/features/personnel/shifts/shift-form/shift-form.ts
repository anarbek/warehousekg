...
export class ShiftForm implements OnInit {
  ...
  protected readonly title = this.isEdit ? 'Редактировать смену' : 'Новая смена';
  protected formData: any = { code: '', name: '', startTime: '08:00', endTime: '17:00', isActive: true };
  protected readonly formItems: any[] = [
    ...
    { dataField: 'startTime', editorType: 'dxTextBox', label: { text: 'Начало (ЧЧ:ММ)' }, isRequired: true, editorOptions: { stylingMode: 'outlined' } },
    { dataField: 'endTime', editorType: 'dxTextBox', label: { text: 'Конец (ЧЧ:ММ)' }, isRequired: true, editorOptions: { stylingMode: 'outlined' } },
  ...
  ngOnInit() {
    ...
    this.svc.getShiftById(this.editId!).subscribe({
  ...
  submit() {
    ...
    if (this.isEdit) {
      (this.svc.updateShift(this.editId!, r) as any).subscribe({ next: done, error: fail });
    } else {
      (this.svc.createShift(r) as any).subscribe({ next: done, error: fail });
    }
  }
