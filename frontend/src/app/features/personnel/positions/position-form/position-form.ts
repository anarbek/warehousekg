...
export class PositionForm implements OnInit {
  ...
  protected readonly title = this.isEdit ? 'Редактировать должность' : 'Новая должность';
  ...
  ngOnInit() {
    ...
    this.svc.getPositionById(this.editId!).subscribe({
  ...
  submit() {
    ...
    if (this.isEdit) {
      (this.svc.updatePosition(this.editId!, r) as any).subscribe({ next: done, error: fail });
    } else {
      (this.svc.createPosition(r) as any).subscribe({ next: done, error: fail });
    }
  }
