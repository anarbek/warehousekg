...
export class PickingForm implements OnInit {
  ...
  protected formData:any={number:'',warehouseId:'',reference:'',notes:'',plannedPickDate:null,employeeId:null};
  ...
  submit(){this.saving.set(true);this.err.set(null);
    this.svc.createPickOrder({number:this.formData.number,warehouseId:this.formData.warehouseId,reference:this.formData.reference||null,notes:this.formData.notes||null,plannedPickDate:this.formData.plannedPickDate?new Date(this.formData.plannedPickDate).toISOString():null,employeeId:this.formData.employeeId||null,lines:this.form.getRawValue().lines.map((l:any)=>({inventoryItemId:l.inventoryItemId!,quantity:Number(l.quantity),warehouseLocationId:this.toGuid(l.warehouseLocationId)}))}).subscribe({next:()=>{this.saving.set(false);void this.router.navigate(['..'],{relativeTo:this.route})},error:(e)=>{this.toast.showSave(e);this.saving.set(false)}})}
