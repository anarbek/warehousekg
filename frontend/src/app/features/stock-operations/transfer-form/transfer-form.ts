...
export class TransferForm implements OnInit {
  ...
  submit(){this.saving.set(true);this.err.set(null);
    this.svc.createStockTransfer({number:this.formData.number,sourceWarehouseId:this.formData.sourceWarehouseId,destinationWarehouseId:this.formData.destinationWarehouseId,notes:this.formData.notes||null,lines:this.form.getRawValue().lines.map((l:any)=>({inventoryItemId:l.inventoryItemId!,quantity:Number(l.quantity)}))}).subscribe({next:()=>{this.saving.set(false);void this.router.navigate(['..'],{relativeTo:this.route})},error:(e)=>{this.toast.showSave(e);this.saving.set(false)}})}
