...
@Component({selector:'app-stock-operation-detail',imports:[FormsModule,DatePipe,LowerCasePipe,DxDataGridModule,DxButtonModule,DxProgressBarModule,DxTextBoxModule],templateUrl:'./stock-operation-detail.html',styleUrl:'./stock-operation-detail.scss'})
export class StockOperationDetail implements OnInit {
  ...
  complete(){this.saving.set(true);const o=(this.op==='receiving'?this.svc.completeStockReceipt(this.id):this.op==='picking'?this.svc.completePickOrder(this.id):this.op==='packing'?this.svc.completePackOrder(this.id):this.svc.cancelStockTransfer(this.id))as Observable<void>;o.subscribe({next:()=>{this.saving.set(false);this.load()},error:()=>{this.saving.set(false)}})}
